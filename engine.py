...
class ServerEngine:
    ...
    def overwrite_fen(self, fen, side):
        ...
        return Response.success(
            f"Overwrite FEN success. Current active FEN: [{str(self.board.fen())}]"
        )
    ...
    def use_this_fen(self, fen):
        ...
        return Response.success(
            f"Use this FEN success. Current active FEN: [{str(self.board.fen())}]"
        )

    @safe_response
    def move(self, notation):
        assert (
            len(notation) == 4 or len(notation) == 5
        ), f"Move notation must 4 or 5, got {notation}"
    ...
    @safe_response
    def on_get_movement_response(self, engine_result):
        message = ""

        if engine_result.resigned:
            message = "WARNING - Engine resigned. Trying to reinitialize engine again."
            self.initialize_engine()

        return Response.move(
            movement=engine_result.move.uci(),
            score=str(engine_result.info["score"].relative),
            is_resigned=engine_result.resigned,
            is_draw_offered=engine_result.draw_offered,
            message=message,
        )
...
class Response:
    @staticmethod
    def move(
        movement: str, score: str, is_resigned: bool, is_draw_offered: bool, message=""
    ):
        return {
            "success": 1,
            "message": message,
